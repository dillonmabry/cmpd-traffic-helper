...
class RestService(object):
    ...
    def get(self, params):
        ...
        """
        try:
            r = self.session.request(method='get', url=self.endpoint, params=params)
            if (r.status_code == requests.codes.ok):
                self.logger.info("GET API endpoint request received, endpoint: {0}".format(self.endpoint))
                return r
                ...
                self.logger.error("Status: {0} | Not found".format(r.status_code))
    ...
    def post(self, payload, headers):
        ...
        """
        try:
            r = self.session.request(method='post', url=self.endpoint, data=payload, headers=headers)
            if (r.status_code == requests.codes.ok):
                self.logger.info("POST API endpoint request received, endpoint: {0}".format(self.endpoint))
                return r
            elif (r.status_code == requests.codes.bad_request):
                self.logger.error("Status: {0} | Bad Request. Check API Key or connectivity".format(r.status_code))
                raise Exception("Bad Request")
